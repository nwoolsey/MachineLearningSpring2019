...
#            branches_tree[0,j] = -1*values[np.argmax(counts)]-1
            branches_tree[0,j] = -1*np.round( np.sum(labels)/np.size(labels))-1
#            if bt1 !=branches_tree[0,j]:
#                print('stop ',branches_tree[0,j],' ', bt1)
        elif np.size(np.unique(L)) == 1 or TreeLoc == TreeDepth or np.size(attr_active) == 0 :
#            (values,counts) = np.unique(L,return_counts=True)
#            branches_tree[0,j] = -1*values[np.argmax(counts)]-1
            branches_tree[0,j] = -1*np.round( np.sum(L)/np.size(L))-1
...
bag_size = 250
num_bags = 1000
attr_ss_size = 4 # The number of random samples considered at each split
num_samples = 100
sample_size = 1000
...
#        print(j)
        ...
        attr_tree, branches_tree, next_branch = ID3_hw1(L[train_data_indeces],label_size,terms_ss[train_data_indeces],attr_size,attr_active,MeasType,1,TreeDepth,next_branch,attr_ss_size)

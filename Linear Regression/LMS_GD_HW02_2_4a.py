...
while t < 1e4 and tol_acc == 0 :
    ...
    grad = np.zeros((num_attr+1,))
    
    for j in range(num_attr+1) :
        for i in range(np.shape(train_data)[0]) :
            if j == num_attr :
                grad[j] = grad[j] - (train_data[i,num_attr] - np.dot(w,np.hstack((train_data[i,0:num_attr], 1))))
            else :
                grad[j] = grad[j] - (train_data[i,num_attr] - np.dot(w,np.hstack((train_data[i,0:num_attr], 1))) )*train_data[i,j]
